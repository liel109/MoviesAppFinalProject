...
class MoviesRepository @Inject constructor(private val remoteDataSource: MoviesRemoteDataSource,
    ...
    fun getMoviesListByTitle(title: String) =
        DataUtils.fetchRemoteData { remoteDataSource.getMoviesByTitle(title) }

    fun getMoviesByGenre(genre: String) =
        DataUtils.fetchRemoteData { remoteDataSource.getMoviesByGenres(genre) }

    fun addMovie(id: Int): LiveData<Resource<MovieItem>> =
        DataUtils.fetchDataAndSave(
    ...
    suspend fun insertMovie(movieItem: MovieItem) {
        localDataSource.addMovie(movieItem)
    }
    ...
    suspend fun deleteMovie(movieItem: MovieItem) {
        localDataSource.deleteMovie(movieItem)
    }

    suspend fun updateMovieFavoriteStatus(id: Int, newStatus: Boolean) {
        localDataSource.updateMovieFavoriteStatus(id, newStatus)
    }

    suspend fun updateMovieNotes(id: Int, newNote: String) {
        localDataSource.updateMovieNotes(id, newNote)
    }
}